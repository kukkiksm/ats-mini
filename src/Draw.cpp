...
// Draw S-meter
//
static void drawSMeter(int strength,  uint8_t snr, int x, int y)
{
  ...
  spr.setTextDatum(TL_DATUM);
  spr.setTextColor(TH.rds_text, TH.bg);
  //spr.drawString("SNR", 292, 100, 2);
  spr.drawString("SNR", 276, 89, 2);
  //spr.drawFloat(snr, x, 260, 106);
  spr.drawFloat(snr, x, 275, 102);
}
...
void drawLayoutDefault(const char *statusLine1, const char *statusLine2)
{
  ...
  //drawSMeter(getStrength(rssi), METER_OFFSET_X, METER_OFFSET_Y);
    drawSMeter(getStrength(rssi), snr, METER_OFFSET_X, METER_OFFSET_Y);
...
void drawScreen(const char *statusLine1, const char *statusLine2)
{
  ...
  aqiStr = "AQI: " + String(aqiValue) + " PM25: " + String(pm25Value) + " ug/m3";

  switch (roundDisplay)
  {
  case 1:
    spr.drawString(weatherStr, 100, 23, 2);
    break;
  case 2:
    spr.drawString(tempStr + " " + rainStr, 100, 23, 2);
    break;
  case 3:
    spr.drawString(windStr, 100, 23, 2);
    break;
  case 4:
    spr.drawString(aqiStr, 100, 23, 2);
    break;
  case 5:
    spr.drawString(forecastText[0], 100, 23, 2);
    break;
  case 6:
    spr.drawString(forecastText[1], 100, 23, 2);
    break;
  case 7:
    spr.drawString(forecastText[2], 100, 23, 2);
    break;
  default:
